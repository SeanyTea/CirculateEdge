...
def detect_image(weights,
    ...
    while True:
        ...
        depth_segmentation = np.zeros((depth_colormap.shape[0],depth_colormap.shape[1]),np.uint8)
        ...
        if len(result_boxes) > 0:
            ...
            if len(outputs[0]) > 0:
                for j, (output, conf) in enumerate(zip(outputs[0], det[:,:4])):
                    ...
                    label = (f'{id} {names[c]}')
                    depth_segmentation[int(bboxes[0][1]):int(bboxes[0][3]),int(bboxes[0][0]):int(bboxes[0][2])] = 255
                    ...
                    hAngle = ((center_of_mass_x-width/2)/(width/2))*(HFOV/2)
                    vAngle = ((640-height/2)/(height/2))*(VFOV/2)
                    yDistance = depth_frame.get_distance(center_of_mass_x,center_of_mass_y)*math.cos((vAngle+15)*math.pi/180)
                    xDistance = depth_frame.get_distance(center_of_mass_x,center_of_mass_y)*math.sin(hAngle*math.pi/180)
                    ...
                    mask = depth_segmentation
                    
                    depth_gray = cv2.cvtColor(depth_colormap,cv2.COLOR_BGR2GRAY)
                    res = cv2.bitwise_and(depth_gray,depth_gray,mask = mask)
